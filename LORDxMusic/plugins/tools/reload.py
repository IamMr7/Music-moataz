...
from LORDxMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from LORDxMusic.utils.decorators import ActualAdminCB, AdminActual, language
from LORDxMusic.utils.formatters import alpha_to_int, get_readable_time
...
@app.on_message(
    filters.command(["/reload", "ريلود", "رفرش"]) & filters.group & ~BANNED_USERS
)
@language
...
@app.on_message(filters.command(["ريبوت"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await LORDy.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await LORDy.stop_stream_force(chat_id)
        except:
            pass
...
async def close_menu(_, query: CallbackQuery):
    try:
        ...
        umm = await query.message.reply_text(
            f"● تـم الاغـلاق بـواسـطـة : {query.from_user.mention}"
        )
